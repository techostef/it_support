...
function CurrentProjectID() {
	...
	return "{2D23B2F1-9107-4245-BBC7-F6E2C1DB634A}";
}
...
class cBreadcrumb {
	...
	function __construct() {
		global $Language;
		$this->Links[] = array("home", "HomePage", "karyawanlist.php", "ewHome", "", FALSE); // Home
	}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						if (is_null($rs->fields('id_user_level'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('id_user_level'))); // Load User Level
						}
						$this->SetupUserLevel();
