...
class ckaryawan extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('karyawan', 'karyawan', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// username
		$this->username = new cField('karyawan', 'karyawan', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->username->Sortable = TRUE; // Allow sort
		$this->fields['username'] = &$this->username;

		// password
		$this->password = new cField('karyawan', 'karyawan', 'x_password', 'password', '`password`', '`password`', 200, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'PASSWORD');
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// no_telp
		$this->no_telp = new cField('karyawan', 'karyawan', 'x_no_telp', 'no_telp', '`no_telp`', '`no_telp`', 200, -1, FALSE, '`no_telp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_telp->Sortable = TRUE; // Allow sort
		$this->fields['no_telp'] = &$this->no_telp;

		// alamat
		$this->alamat = new cField('karyawan', 'karyawan', 'x_alamat', 'alamat', '`alamat`', '`alamat`', 201, -1, FALSE, '`alamat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->alamat->Sortable = TRUE; // Allow sort
		$this->fields['alamat'] = &$this->alamat;

		// id_user_level
		$this->id_user_level = new cField('karyawan', 'karyawan', 'x_id_user_level', 'id_user_level', '`id_user_level`', '`id_user_level`', 3, -1, FALSE, '`id_user_level`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->id_user_level->Sortable = TRUE; // Allow sort
		$this->id_user_level->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->id_user_level->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->id_user_level->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_user_level'] = &$this->id_user_level;
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->id_user_level->setDbValue($rs->fields('id_user_level'));
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->id_user_level->Exportable) $Doc->ExportCaption($this->id_user_level);
				} else {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->id_user_level->Exportable) $Doc->ExportField($this->id_user_level);
					} else {
